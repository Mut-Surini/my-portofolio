  ...
  setupCarousels() {
    ...
    carouselContainers.forEach((container, index) => {
      ...
      dots.forEach((dot, dotIndex) => {
        dot.addEventListener("click", () => this.goToSlide(carousel, dotIndex));
      });
  ...
  updateParallax() {
    ...
    if (heroBg && heroContent) {
      ...
      const heroOpacity = Math.max(
        0,
        Math.min(1, 1 - (this.scrollY / heroHeight) * 1.5)
      );

      heroBg.style.transform = `translateY(${this.scrollY * 0.5}px)`;
      heroContent.style.opacity = heroOpacity;
      heroContent.style.transform = `translateY(${-this.scrollY * 0.2}px)`;
    }
    ...
    if (experienceBg && experienceSection) {
      ...
      experienceBg.style.transform = `translateY(${
        (this.scrollY - experienceOffset) * 0.1
      }px)`;
    }
    ...
    if (projectsBg && projectsSection) {
      ...
      projectsBg.style.transform = `translateY(${
        (this.scrollY - projectsOffset) * 0.4
      }px)`;
    }
    ...
    if (contactBg && contactSection) {
      ...
      contactBg.style.transform = `translateY(${
        (this.scrollY - contactOffset) * 0.3
      }px)`;
    }
  }
